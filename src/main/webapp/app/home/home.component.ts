...
export class HomeComponent implements OnInit, OnDestroy {
    ...
    public deploymentGroups: Map<string, { gitData: object[], collapsed: boolean, ckeckMK_status: string }> =
             new Map<string, { gitData: object[], collapsed: boolean, ckeckMK_status: string }>();
    public deployments: Map<string, { showEnvs: boolean, apps: { gitData: Map<string, Object[]>, collapsed: boolean } }>
            = new Map<string, { showEnvs: false, apps: { gitData: Map<string, Object[]>, collapsed: false } }>();
    ...
    ngOnInit() {
        ...
        this.gitlab.getLastestBuilld().subscribe((buildData) => {
            this.latestBuild = buildData[0];
        });
    ...
    getDeploymentData() {
        ...
        this.subscription = this.gitlab.retrieveDeployments(50).subscribe((deployment) => {
            ...
            deployment.forEach((e) => {
                ...
                if (this.deployments.get(environment[0]) == null) {
                    ...
                    this.deployments.set(environment[0], {
                        showEnvs: false,
                        apps: {gitData: new Map<string, Object[]>(), collapsed: false}
                    });
                }
    ...
    downloadProps() {
        ...
        this.gitlab.downloadFile('')
            ...
            .subscribe((res: Response) => {
                const blob = new Blob([atob(res['content'])], {type: 'application/zip'});
